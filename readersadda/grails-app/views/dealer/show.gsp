	...
	<body>
		...
		<div id="show-dealer" class="content scaffold-show" role="main">
			...
			<ol class="property-list dealer">
				...
				<g:if test="${dealerInstance?.user}">
				<li class="fieldcontain">
					<span id="user-label" class="property-label"><g:message code="dealer.user.label" default="User" /></span>
					
						<span class="property-value" aria-labelledby="user-label"><g:link controller="user" action="show" id="${dealerInstance?.user?.id}">${dealerInstance?.user?.encodeAsHTML()}</g:link></span>
